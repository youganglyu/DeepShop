...
def auto_eval_by_gpt4v_submetric_mp_sort(start, end,test_name,test_data, api_model, img_num,return_dict):
    out_list_local = []
    ...
    for i in range(start, end):
        ...
        user_prompt_tmp = USER_PROMPT.replace('<task>', test_data[i]['sort'])
...
if __name__ == '__main__':
    ...
    for idx in range(num_processes):
        ...
        p = multiprocessing.Process(target=auto_eval_by_gpt4v_submetric_mp_sort, args=(
    ...
    print(f"Sort ACC: {accuracy:.2f}")
    domains = np.array(out_data)
    np.save('../../results/'+args.test_name +"_sort"+ '.npy', out_data)
