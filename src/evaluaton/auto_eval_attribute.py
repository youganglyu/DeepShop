...
def auto_eval_by_gpt4v_submetric_mp_attribute(start, end,test_name,test_data, api_model, img_num,return_dict):
    out_list_local = []
    ...
    for i in range(start, end):
        process_dir = os.path.join('../s/results/'+test_name+'/', 'task' + test_data[i]['id'])
        print(f'--------------------- {process_dir} ---------------------')
        ...
        user_prompt_tmp = USER_PROMPT.replace('<task>', test_data[i]['attribute'])
...
if __name__ == '__main__':
    ...
    for idx in range(num_processes):
        ...
        p = multiprocessing.Process(target=auto_eval_by_gpt4v_submetric_mp_attribute, args=(
    ...
    print(f"Attribute Acc: {accuracy:.2f}")
    domains = np.array(out_data)
    np.save('../../results/'+args.test_name +"_attribute"+ '.npy', out_data)
